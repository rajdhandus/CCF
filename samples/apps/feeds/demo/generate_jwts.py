...
def generate_npm_feed(data_dir: Path):
    ...
    for pkg in pkgs:
        ...
        jwt = infra.crypto.create_jwt(pkg_info, jwt_key_priv_pem, key_id=name, cert_pem=jwt_cert_pem)
...
def generate_contoso_feed(data_dir: Path):
    ...
    for npm_receipt_path in npm_feed_dir.glob('*.receipt.json'):
        ...
        jwt = infra.crypto.create_jwt(audit, jwt_key_priv_pem, key_id=name, cert_pem=jwt_cert_pem)
        jwt_path = feed_dir / f'{subject}.jwt'.replace('/', '_')
        print(f'Writing {jwt_path}')
        with open(jwt_path, 'w') as f:
            f.write(jwt)
